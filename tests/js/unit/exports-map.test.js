...
// The two barrels are exposed via the root and "./extras" keys, NOT via a
// per-module subpath. Every other module must have its own subpath.
const BARRELS = new Set(["index.js", "extras.js"]);
...
function subpathModuleMap() {
  const map = new Map();
  // Barrel keys map to index.js / extras.js, not to a per-module subpath.
  const BARREL_KEYS = new Set([".", "./extras", "./assets/*"]);
...
describe("package.json exports map", () => {
  ...
  it("the barrels are exposed via the root and ./extras keys", () => {
